...
namespace ParadoxReader
{
    ...
    internal static class SecondaryIndexDiscovery
    {
        public static List<SecondaryIndexInfo> Discover(
            string dbFilePath, ParadoxFile.FieldInfo[] allFields)
        {
            ...
            TryScanPattern(dir, name, "X",  padWidth: 2, from: 0, to: 99,  allFields, result);

            // Incremental secondary indexes: .XG0 - .XG9
            TryScanPattern(dir, name, "XG", padWidth: 1, from: 0, to: 9,   allFields, result);

            // Maintained-field companions to non-incremental indexes: .Y00 - .Y99
            TryScanPattern(dir, name, "Y",  padWidth: 2, from: 0, to: 99,  allFields, result);

            // Maintained-field companions to incremental indexes: .YG0 - .YG9
            TryScanPattern(dir, name, "YG", padWidth: 1, from: 0, to: 9,   allFields, result);
        ...
        private static void TryScanPattern(
            ...
            ParadoxFile.FieldInfo[] allFields, List<SecondaryIndexInfo> result)
        {
            for (int i = from; i <= to; i++)
            {
                ...
                try
                {
                    var info = ReadIndexHeader(path, allFields);
        ...
        private static SecondaryIndexInfo ReadIndexHeader(
            string indexFilePath, ParadoxFile.FieldInfo[] allFields)
        {
            using (var f = new ParadoxFile(indexFilePath))
            {
                ...
                if (startField >= 0 && startField < allFields.Length)
                {
                    ...
                    // Multi-field secondary indexes index consecutive fields starting
                    // at indexFieldNumber.
                    for (int i = 0; i < f.FieldCount && (startField + i) < allFields.Length; i++)
                    {
